...
const matriz1 = [
  ...
  const sumarMatrices = (array1 array2: number[][]): number[][] => {
    let patatas: [] = []
    let resultado: any[] = []
    let resultado2: any[] = []
    ...
    array1.map((valor) => {
      ...
      valor.map((clave) => {
        resultado.push("el valor 1 dentro del segundo map", clave)
        // resultado= []
      })
    })
    array2.map((valor) => {
      ...
      valor.map((clave) => {
        resultado2.push("el valor 2 dentro del segundo map", clave)
        // resultado = []
      })
    })
    ...
    array1.map((x, i) => {
      let otracosa = x[i]
      otracosa = resultado[i] + resultado2[i]
      patatas.push(otracosa)
    })
